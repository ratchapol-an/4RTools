...
# Stage in %TEMP% first — zipping under dist\ often fails (Explorer preview, Defender, indexer locking .mui files).
$tempStage = Join-Path $env:TEMP ("PartyWingBuffPortable_" + [System.Guid]::NewGuid().ToString("N"))
try {
    ...
    $cmd = Join-Path $tempStage "Run-PartyWingBuffTools.cmd"
    @"
@echo off
setlocal
start "" "%~dp0PartyWingBuffTools.exe"
"@ | Set-Content -Path $cmd -Encoding ASCII
    ...
    for ($i = 1; $i -le 8; $i++) {
        try {
            ...
            # Zip contents so root of archive is files/folders, not a single parent folder name.
    ...
    if (-not $zipOk) {
        Write-Error "Could not create zip after retries. Close any Explorer windows on dist\, disable preview pane, or retry."
    }
...
Write-Host "Run PartyWingBuffTools.exe as Administrator for memory reads."
